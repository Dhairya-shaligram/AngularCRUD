...
export class DataListComponent implements AfterViewInit {
  ...
  initList() {
    ...
    if (this.tableType == TableType.Pagination) {
      Object.keys(this.pagination).forEach((key) => {
        this.apiDetails.requestData[key] =  this.pagination[key as keyof PaginationModel];
      });
    }
  ...
  setTablePagination() {
    if (this.isFirstTime) {
      ...
      // Add a null check for paginator
      if (this.paginator) {
  ...
  getColumnDetails(type: string, columnDef: string): any {
    ...
    const column = this.columns.find((d) => d.columnDef === columnDef);
    if (!column) {
      return ''; // handle case where column is not found
    }
  
    if (type === 'title') {
      return column.title; // Access title if column exists
    } else if (type === 'button') {
      return column.buttons; // Access buttons if column exists
    }
